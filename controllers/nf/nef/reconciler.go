...
// Reconciles a NEF NFDeployment resource
type NEFDeploymentReconciler struct {
	client.Client
...
func (r *NEFDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := log.FromContext(ctx).WithValues("NFDeployment", req.NamespacedName, "NF", "NEF")

	nfDeployment := new(nephiov1alpha1.NFDeployment)
	err := r.Client.Get(ctx, req.NamespacedName, nfDeployment)
	if err != nil {
		if k8serrors.IsNotFound(err) {
			log.Info("NEF NFDeployment resource not found, ignoring because object must be deleted")
			return reconcile.Result{}, nil
		}
		log.Error(err, "Failed to get NEF NFDeployment")
...
func (r *NEFDeploymentReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		// Watch NFDeployment with provider nef.sdcore.io
		For(&nephiov1alpha1.NFDeployment{}).
		// Filter NFDeployments with provider nef.sdcore.io
		WithEventFilter(controllers.ProviderFilter("nef.sdcore.io")).
		Complete(r)
} 