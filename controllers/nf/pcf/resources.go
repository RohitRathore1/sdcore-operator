...
func createDeployment(log logr.Logger, configMapVersion string, pcfDeployment *nephiov1alpha1.NFDeployment) (*appsv1.Deployment, error) {
	namespace := pcfDeployment.Namespace
	instanceName := pcfDeployment.Name
	spec := pcfDeployment.Spec
...
func createConfigMap(log logr.Logger, pcfDeployment *nephiov1alpha1.NFDeployment) (*apiv1.ConfigMap, error) {
	namespace := pcfDeployment.Namespace
	instanceName := pcfDeployment.Name
	...
	configMap := &apiv1.ConfigMap{
		...
		Data: map[string]string{
			"wrapper.sh": `#!/bin/bash
# Configuration script for PCF
set -x

# Create a config directory for PCF
mkdir -p /free5gc/config/pcfcfg.yaml

# Copy the default config
cp /free5gc/config/pcfcfg.conf /free5gc/config/pcfcfg.yaml
...
func createService(log logr.Logger, pcfDeployment *nephiov1alpha1.NFDeployment) (*apiv1.Service, error) {
	namespace := pcfDeployment.Namespace
	instanceName := pcfDeployment.Name
