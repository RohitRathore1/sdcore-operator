...
func createConfigMap(log logr.Logger, smfDeployment *nephiov1alpha1.NFDeployment) (*apiv1.ConfigMap, error) {
	namespace := smfDeployment.Namespace
	instanceName := smfDeployment.Name
	...
	configMap := &apiv1.ConfigMap{
		...
		Data: map[string]string{
			"wrapper.sh": `#!/bin/bash
# Configuration script for SMF
set -x

# Create a config directory for SMF
mkdir -p /free5gc/config/smfcfg.yaml

# Copy the default config
cp /free5gc/config/smfcfg.conf /free5gc/config/smfcfg.yaml
...
func createService(log logr.Logger, smfDeployment *nephiov1alpha1.NFDeployment) (*apiv1.Service, error) {
	namespace := smfDeployment.Namespace
	instanceName := smfDeployment.Name
...
func createNetworkAttachmentDefinitionNetworks(name string, spec *nephiov1alpha1.NFDeploymentSpec) (string, error) {
	...
	// For now, returning a simple example for SMF
	return `[
    {
        "name": "n4-net",
        "interface": "n4"
