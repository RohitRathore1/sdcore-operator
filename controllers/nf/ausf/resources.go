...
func createDeployment(log logr.Logger, configMapVersion string, ausfDeployment *nephiov1alpha1.NFDeployment) (*appsv1.Deployment, error) {
	namespace := ausfDeployment.Namespace
	instanceName := ausfDeployment.Name
	spec := ausfDeployment.Spec
...
func createConfigMap(log logr.Logger, ausfDeployment *nephiov1alpha1.NFDeployment) (*apiv1.ConfigMap, error) {
	namespace := ausfDeployment.Namespace
	instanceName := ausfDeployment.Name
	...
	configMap := &apiv1.ConfigMap{
		...
		Data: map[string]string{
			"wrapper.sh": `#!/bin/bash
# Configuration script for AUSF
set -x

# Create a config directory for AUSF
mkdir -p /free5gc/config/ausfcfg.yaml

# Copy the default config
cp /free5gc/config/ausfcfg.conf /free5gc/config/ausfcfg.yaml
...
func createService(log logr.Logger, ausfDeployment *nephiov1alpha1.NFDeployment) (*apiv1.Service, error) {
	namespace := ausfDeployment.Namespace
	instanceName := ausfDeployment.Name
