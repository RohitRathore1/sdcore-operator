...
func createConfigMap(log logr.Logger, amfDeployment *nephiov1alpha1.NFDeployment) (*apiv1.ConfigMap, error) {
	...
	configMap := &apiv1.ConfigMap{
		...
		Data: map[string]string{
			"wrapper.sh": `#!/bin/bash
# Configuration script for AMF
set -x

# Create a config directory for AMF
mkdir -p /free5gc/config/amfcfg.yaml

# Copy the default config
cp /free5gc/config/amfcfg.conf /free5gc/config/amfcfg.yaml
...
func createService(log logr.Logger, amfDeployment *nephiov1alpha1.NFDeployment) (*apiv1.Service, error) {
	namespace := amfDeployment.Namespace
	instanceName := amfDeployment.Name
...
func createNetworkAttachmentDefinitionNetworks(name string, spec *nephiov1alpha1.NFDeploymentSpec) (string, error) {
	...
	// For now, returning a simple example for AMF
	return `[
    {
        "name": "n1-net",
        "interface": "n1"
