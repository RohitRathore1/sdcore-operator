...
func createDeployment(log logr.Logger, configMapVersion string, udrDeployment *nephiov1alpha1.NFDeployment) (*appsv1.Deployment, error) {
	namespace := udrDeployment.Namespace
	instanceName := udrDeployment.Name
	spec := udrDeployment.Spec
...
func createConfigMap(log logr.Logger, udrDeployment *nephiov1alpha1.NFDeployment) (*apiv1.ConfigMap, error) {
	namespace := udrDeployment.Namespace
	instanceName := udrDeployment.Name
	...
	configMap := &apiv1.ConfigMap{
		...
		Data: map[string]string{
			"wrapper.sh": `#!/bin/bash
# Configuration script for UDR
set -x

# Create a config directory for UDR
mkdir -p /free5gc/config/udrcfg.yaml

# Copy the default config
cp /free5gc/config/udrcfg.conf /free5gc/config/udrcfg.yaml
...
func createService(log logr.Logger, udrDeployment *nephiov1alpha1.NFDeployment) (*apiv1.Service, error) {
	namespace := udrDeployment.Namespace
	instanceName := udrDeployment.Name
