...
func createDeployment(log logr.Logger, configMapVersion string, udmDeployment *nephiov1alpha1.NFDeployment) (*appsv1.Deployment, error) {
	namespace := udmDeployment.Namespace
	instanceName := udmDeployment.Name
	spec := udmDeployment.Spec
...
func createConfigMap(log logr.Logger, udmDeployment *nephiov1alpha1.NFDeployment) (*apiv1.ConfigMap, error) {
	namespace := udmDeployment.Namespace
	instanceName := udmDeployment.Name
	...
	configMap := &apiv1.ConfigMap{
		...
		Data: map[string]string{
			"wrapper.sh": `#!/bin/bash
# Configuration script for UDM
set -x

# Create a config directory for UDM
mkdir -p /free5gc/config/udmcfg.yaml

# Copy the default config
cp /free5gc/config/udmcfg.conf /free5gc/config/udmcfg.yaml
...
func createService(log logr.Logger, udmDeployment *nephiov1alpha1.NFDeployment) (*apiv1.Service, error) {
	namespace := udmDeployment.Namespace
	instanceName := udmDeployment.Name
